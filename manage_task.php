		...
		<div  class="form-group">
			...
			<select class="custom-select custom-select-sm" name="task_member">	
				...
				$task_member = $result["task_member"];
              	...
              	$query2 = $conn->query("SELECT user_ids as name FROM project_list where id= ".$_GET['pid']);
				$result2= $query2->fetch_assoc();
				$user_ids = $result2["name"];

              	$employees = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM users where id in ($user_ids) order by concat(firstname,' ',lastname) asc");
              	while($row= $employees->fetch_assoc()):
              	?>				
              	<option value="<?php echo $row['id'] ?>" <?php echo isset($user_ids) && $row["id"] == $task_member ? "selected" : '' ?>><?php echo ucwords($row['name']) ?></option>
              	<?php endwhile; ?>
              </select>
		</div>
	</form>
